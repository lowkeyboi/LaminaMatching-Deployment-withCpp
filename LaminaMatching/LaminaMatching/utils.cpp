...
vector<Point> Get_pairs_idx(vector<Point>& points_left,
    ...
    cv::Mat& image)
{
    ...
    std::string output_paf = model.getOutputsInfo().begin()->first;
    std::string output_pcm = (++model.getOutputsInfo().begin())->first;
    std::string output_bbox = (++(++model.getOutputsInfo().begin()))->first;

    InferenceEngine::Blob::Ptr input = infer_request.GetBlob(input_name);

    InferenceEngine::Blob::Ptr output_PAF = infer_request.GetBlob(output_paf);
    InferenceEngine::Blob::Ptr output_PCM = infer_request.GetBlob(output_pcm);
    InferenceEngine::Blob::Ptr output_Bbox = infer_request.GetBlob(output_bbox);
    ...
    Get_all_points(PCM_LEFT, PCM_RIGHT, points_left, points_right, ori_width, ori_height);
    ...
    PAF_finetune(PAF_MAP_X, PAF_MAP_Y, PAF_vector_size);

    vector<Point> pairs_point_idx;
    vector<vector<Point>> pair;
    pairs_point_idx = Get_pairs_idx(points_left, points_right, PAF_MAP_X, PAF_MAP_Y);
